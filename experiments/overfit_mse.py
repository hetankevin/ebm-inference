#!/usr/bin/env python3
"""Benchmark multiple regressors with Optuna on a noisy sinusoid dataset and plot MSE curves."""
import argparse
import json
import os
import sys
...
def make_noisy_sinusoid(n: int, noise_std: float, seed: int) -> Tuple[np.ndarray, np.ndarray]:
    """Generate samples from a noisy sinusoid with additive Gaussian noise."""
...
def load_noisy_sinusoid(n_samples: int, noise_std: float, seed: int) -> Dataset:
    X, y = make_noisy_sinusoid(n_samples, noise_std, seed)
    frame = pd.DataFrame({"x": X[:, 0]})
    target = pd.Series(y, name="y")
    return Dataset("Noisy Sinusoid", frame, target)


DATASET_LOADERS = {
    "noisy_sinusoid": load_noisy_sinusoid,
}
...
def main():
    ...
    parser.add_argument("--datasets", nargs="*", default=["noisy_sinusoid"], choices=list(DATASET_LOADERS.keys()))
    ...
    parser.add_argument("--plot", type=str, default="plots/overfit_mse.png")
