...
class InferableEBMRegressor(RegressorMixin, EBMModel):
    ...
    def predict(self, X, init_score=None):
        ...
        Returns predictions according to the paper formula:
        f̂(x) = β₀ + 2∑_{k=1}^p f_k^(B)(x^(k))
        """
        import numpy as np
        X = np.asarray(X)
        ...
        for j in range(p):
            ...
            # Apply factor of 2 as per paper: f̂(x) = β₀ + 2∑_{k=1}^p f_k^(B)(x^(k))
            out += 2.0 * term_arr[j][new_bins]
